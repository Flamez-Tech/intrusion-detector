...
import { Download, RotateCcw, Settings, Database, FileText } from "lucide-react"
...
export function AdvancedControls() {
  const { events, alerts, status, clearAlerts } = useDetectionData()
  ...
  const exportData = async (format = "json") => {
    ...
    try {
      ...
      link.href = url
      link.download = `intrusion-data-${new Date().toISOString().split("T")[0]}.${format}`
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>System management, data export, and configuration options</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          ...
          {/* Generate Report */}
          <Button variant="outline" onClick={generateReport} className="w-full bg-transparent">
            <FileText className="mr-2 h-4 w-4" />
            Generate Report
          </Button>

          {/* Reset System */}
          <Button variant="outline" onClick={resetSystem} disabled={loading} className="w-full bg-transparent">
          ...
          <Dialog open={configDialogOpen} onOpenChange={setConfigDialogOpen}>
            ...
            <DialogContent className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>Advanced system settings and preferences</DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="max-events">Max Events to Store</Label>
                  <Input id="max-events" type="number" defaultValue="1000" min="100" max="10000" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="update-interval">Update Interval (ms)</Label>
                  <Input id="update-interval" type="number" defaultValue="1000" min="500" max="10000" />
                </div>
        ...
        <div className="pt-4 border-t">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <div className="font-mono">{new Set(events.map((e) => e.sourceIP)).size}</div>
            </div>
          </div>
        </div>
      </CardContent>
