    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          {alerts.length > 0 && (
            <Button variant="outline" size="sm" onClick={clearAlerts} disabled={loading}>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div>
                      ...
                      <div className="font-mono">
                        {alert.affectedIPs.slice(0, 2).join(", ")}
                        {alert.affectedIPs.length > 2 && ` +${alert.affectedIPs.length - 2}`}
                      </div>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Events:</span>
                      <div className="font-mono">{alert.eventCount}</div>
                    </div>
                  </div>
                  ...
                  {alert.features && (
                    <div className="space-y-1">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div>Request Rate: {alert.features.requestRate.toFixed(1)}/s</div>
                        <div>Error Rate: {(alert.features.errorRate * 100).toFixed(1)}%</div>
                        <div>Response Time: {alert.features.responseTime.toFixed(0)}ms</div>
                        <div>Unique IPs: {alert.features.uniqueIPs}</div>
                      </div>
                    </div>
                  )}
                </div>
